...
const DiscountChargesReportForm = ({ isOpen, parentDiscountChargesReportTable, closeIsopen }) => {
    const dispatch = useDispatch();
    const store = useSelector((state) => state)
    const storeListData = store.StoreList?.storeList
    const discountChargeReportStatus = store?.DiscountChargeReportList?.discountChargeReport?.status;
    const discountChargeReportMessage = store?.DiscountChargeReportList?.discountChargeReport?.message;
    const discountChargeReportLorder = store?.DiscountChargeReportList?.loading;
    ...
    const handleDiscountChargesReport = () => {
        parentDiscountChargesReportTable({
    ...
    useEffect(() => {
        if (discountChargeReportStatus === true) {
            ToastHandle('success', discountChargeReportMessage);
            closeIsopenBtn()
        } else if (discountChargeReportStatus === false) {
            ToastHandle('error', discountChargeReportMessage);
        }
    }, [discountChargeReportStatus]);
        ...
        <>
            <Collapse in={isOpen}>
                <div>
                    <Card>
                        <Card.Body>
                            {discountChargeReportLorder ? <MainLoader /> : <Row className="p-3 ">
                                                ...
                                                <Col lg={12}>
                                                    ...
                                                    <Row>
                                                        <Col lg={12} className="d-flex justify-content-center">
                                                            <Button type="submit" className="btn btn-success" onClick={() => handleDiscountChargesReport()}>
                                                                Submit
                                                            </Button>
...
export default DiscountChargesReportForm
