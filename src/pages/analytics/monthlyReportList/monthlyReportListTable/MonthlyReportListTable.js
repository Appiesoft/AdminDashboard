...
import MonthlyWiseForm from '../MothlyWiseForm/MonthlyWiseForm';
import './MonthlyReportListTable.css'
import { useDispatch, useSelector } from 'react-redux';
import { MonthlyReportList } from '../../../../redux/actions';
...
import ShowHide from '../../monthlyReportList/monthlyReportListTable/model/showHideColumns/ShowHide';




const MonthlyReportListTable = ({ TableShowBtn }) => {
    const dispatch = useDispatch();
    const store = useSelector((state) => state);
    const monthlyReportListData = store?.MonthlyReportList?.monthlyReportList?.data

    const paginationValues = store?.MonthlyReportList?.monthlyReportList?.meta?.pagination
    const monthlyReportListLoader = store?.MonthlyReportList;
    ...
    let headers = [];
    if (monthlyReportListData) { headers = DynamicHeaders(monthlyReportListData, ['emp_id']); }
    ...
    const handleGeneratePdf = () => {
        var dataLists = monthlyReportListData
    ...
    const getChecKBoxData = () => {
        const arr = monthlyReportListData
                ...
                <Col xs={12}>
                    <Card>
                        <Card.Body className='pt-0'>
                            ...
                            <Row>
                                <Col>
                                    <MonthlyWiseForm isOpen={isOpen} />
                                </Col>
                            </Row>
                            {monthlyReportListLoader.loading ? <MainLoader /> : <>
                                <Row >
                                    <Col className='overflow-auto '>
                                        <Table className="mb-0 " ref={componentRef} size="sm">
                                            ...
                                            <tbody>
                                                {monthlyReportListData?.map((item, index) => {
                                                    return (
