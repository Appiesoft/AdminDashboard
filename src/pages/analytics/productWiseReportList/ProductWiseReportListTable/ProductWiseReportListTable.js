...
import './ProductWiseReportListTable.css'
import ProductWiseReportForm from '../ProductWiseReportForm/ProductWiseReportForm'
import { useSelector, useDispatch } from 'react-redux';
import { ProductWiseReportList } from '../../../../redux/analytics/productWiseReport/action';
import ShowHide from '../ProductWiseReportListTable/model/showHideColumns/ShowHide';
...
const ProductWiseReportListTable = () => {
    const dispatch = useDispatch()
    const store = useSelector((state) => state)
    const productWiseReportData = store?.ProductWiseReportList?.productWiseReportList?.data
    const [isSortDropdownOpen, setIsSortDropdownOpen] = useState(false);
    const paginationValues = store?.ProductWiseReportList?.productWiseReportList?.meta?.pagination
    const productWiseReportLoader = store.ProductWiseReportList;
    ...
    if (productWiseReportData) {
        headers = DynamicHeaders(productWiseReportData, ['emp_id']);
    }
    ...
    const handleGeneratePdf = () => {
        var dataLists = productWiseReportData
    ...
    const getChecKBoxData = () => {
        const arr = productWiseReportData
    ...
    const parentProductWiseReportTable = (item) => {
    ...
    useEffect(() => {
        dispatch(ProductWiseReportList({
                ...
                <Col xs={12}>
                    <Card>
                        <Card.Body className='pt-0'>
                            ...
                            <Row>
                                <Col>
                                    <ProductWiseReportForm isOpen={isOpen} parentProductWiseReportTable={parentProductWiseReportTable} toggle={toggle} />
                                </Col>
                            </Row>
                            {productWiseReportLoader.loading ? <MainLoader /> : <>
                                <Row >
                                    <Col className='overflow-auto table_container'>
                                        <Table className="mb-0" ref={componentRef} size="sm">
                                            ...
                                            <tbody>
                                                {productWiseReportData?.map((item, index) => {
                                                    return (
...
export default ProductWiseReportListTable
