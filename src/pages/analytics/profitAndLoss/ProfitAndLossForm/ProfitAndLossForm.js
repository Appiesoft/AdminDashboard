...
const ProfitAndLossForm = ({ isOpen, parentProfitAndLossReportTable, closeIsOpen }) => {
    const dispatch = useDispatch();
    const store = useSelector((state) => state)
    const storeListData = store.StoreList?.storeList
    const profitAndLossReportListStatus = store?.ProfitAndLossReportList?.profitAndLossReportList?.status;
    const profitAndLossReportListMessage = store?.ProfitAndLossReportList?.profitAndLossReportList?.message;
    const profitAndLossReportListLorder = store?.ProfitAndLossReportList?.loading
    ...
    const handleProfitAndLossReport = () => {
        parentProfitAndLossReportTable({
    ...
    useEffect(() => {
        if (profitAndLossReportListStatus === true) {
            ToastHandle('success', profitAndLossReportListMessage);
            closeIsOpenBtn();
        } else if (profitAndLossReportListStatus === false) {
            ToastHandle('error', profitAndLossReportListMessage);
        }
    }, [profitAndLossReportListStatus]);
    return (
        <>
            <Collapse in={isOpen}>
                <div>
                    <Card>
                        <Card.Body>
                            {profitAndLossReportListLorder ? <MainLoader /> : <Row className="p-3 ">
                                <Col lg={12}>
                                    ...
                                    <Row>
                                        <Col lg={12} className="d-flex justify-content-center mt-2">
                                            <Button type="submit" className="btn btn-success" onClick={() => handleProfitAndLossReport()}>
                                                Submit
                                            </Button>
