...
import './RetailProductWiseReportListTable.css'
import RetailProductWiseForm from '../RetailProductWiseForm/RetailProductWiseForm'
import { useSelector, useDispatch } from 'react-redux';
import { RetailProductWiseReportListAction } from '../../../../redux/actions';
...
const RetailProductWiseReportListTable = () => {
    const dispatch = useDispatch();
    const store = useSelector((state) => state)
    const reatilProductData = store?.RetailProductWiseReportList?.retailProductReportList?.data
    const paginationValues = store?.RetailProductWiseReportList?.retailProductReportList?.meta?.pagination
    ...
    let headers = [];
    if (reatilProductData) { headers = DynamicHeaders(reatilProductData, ['emp_id']); }
    ...
    const handleGeneratePdf = () => {
        var dataLists = reatilProductData
    ...
    const parentRetailReportTable = (item) => {
    ...
    useEffect(() => {
        dispatch(RetailProductWiseReportListAction({
    ...
    return (
        <div>
            <Row>
                <Col>
                    <h4>Retail Product Wise Reports</h4>
                </Col>
            </Row>
            <Row>
                <Col xs={12}>
                    <Card>
                        <Card.Body className='pt-0'>
                            ...
                            <Row>
                                <Col>
                                    <RetailProductWiseForm isOpen={isOpen} parentRetailReportTable={parentRetailReportTable} closeIsopen={closeIsopen} />
                                </Col>
                            </Row>
                            ...
                            <Row >
                                <Col className='overflow-auto table_container'>
                                    <Table className="mb-0 " size="sm">
                                        ...
                                        <tbody>
                                            {reatilProductData?.map((item, index) => {
                                                return (
...
export default RetailProductWiseReportListTable
