...
const WalletHistoryHearder = ({ isOpen, parentWalletHistoryReportTable, closeIsOpen }) => {
    const dispatch = useDispatch();
    const store = useSelector((state) => state)
    const storeListData = store.StoreList?.storeList
    const walletHistoryReportListStatus = store?.WalletHistoryReportList?.walletHistoryReportList?.status;
    const walletHistoryReportListMessage = store?.WalletHistoryReportList?.walletHistoryReportList?.message;
    const walletHistoryReportListLorder = store?.WalletHistoryReportList?.loading
    ...
    const handleWalletHistoryReport = () => {
        parentWalletHistoryReportTable({
    ...
    useEffect(() => {
        if (walletHistoryReportListStatus === true) {
            ToastHandle('success', walletHistoryReportListMessage);
            closeIsOpenBtn();
        } else if (walletHistoryReportListStatus === false) {
            ToastHandle('error', walletHistoryReportListMessage);
        }
    }, [walletHistoryReportListStatus]);
        ...
        <>
            <Collapse in={isOpen}>
                <div>
                    <Card>
                        <Card.Body>
                            {walletHistoryReportListLorder ? <MainLoader /> : <Row className="p-3 ">
                                <Col lg={12}>
                                    ...
                                    <Row>
                                        <Col lg={12} className="d-flex justify-content-center mt-2">
                                            <Button type="submit" className="btn btn-success" onClick={() => handleWalletHistoryReport()}>
                                                Submit
                                            </Button>
...
export default WalletHistoryHearder