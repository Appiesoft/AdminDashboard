...
import './PaymentTypeListTable.css'
import EditRecordForm from './model/editRecord/EditRecordForm';
...
const PaymentTypeListTable = ({ TableShowBtn }) => {
    const dispatch = useDispatch();
    const store = useSelector(state => state);
    const PaymentTypeLoader = store.PaymentTypeListReducer;
    const PaymentTypeData = store?.PaymentTypeListReducer?.paymentTypeList?.data
    const paginationValues = store?.PaymentTypeListReducer?.paymentTypeList?.meta?.pagination
    const paymentDeleteStatus = store?.PaymentTypeRemove?.status
    const paymentDeleteMessage = store?.PaymentTypeRemove?.message
    ...
    // model
    const openModalEditRecord = (fill, id) => {

        setEditData(id)
        setParentEditRecord(fill, id)
    };
    const closeditModel = (value) => {
        setEditModel(false)
    }
    ...
    useEffect(() => {
        getPaymentType();
        // console.log(parentEditRecord, "sandjksbf")
        // setEditModel(true)
    }, [page, , searchText,]);
    ...
    if (PaymentTypeData) {
        headers = DynamicHeaders(PaymentTypeData, ['image_url']);
    }

    const handleGeneratePdf = () => {
        var dataLists = PaymentTypeData;
        exportPdf(dataLists, headers, 'Payment Type List');
    };
                        ...
                        <Card>
                            <Card.Body className='pt-0'>
                                <Row className='d-flex align-items-center my-2'>
                                    ...
                                    <Col>
                                        <Col lg={12} className="d-flex  px-2 justify-content-end">
                                            <Row>
                                                {isCsvDownload && <CSVDownload filename={"patternList.csv"} data={PaymentTypeData} headers={headers} target="_blank" />}
                                ...
                                {PaymentTypeLoader?.loading ? <Loader /> : <Row>
                                    <Col>
                                        <Table ref={componentRef} className="mb-0 " size="sm">
                                            ...
                                            <tbody>
                                                {PaymentTypeData?.map((record, index) => {
                                                    ...
                                                    return (
                                                        <tr className='align-middle'>
                                                            ...
                                                            <td>{record.method}</td>
                                                            <td>{<img style={{ width: 50 }} src={record.image_url} />}</td>
                                                            <td><Dropdown
                                                                addonType="append"
                                                                ...
                                                                <Dropdown.Menu className='bg-light px-2'>
                                                                    <Dropdown.Item className='bg-light'>
                                                                        ...
                                                                        <OverlayTrigger placement="bottom" overlay={<Tooltip id="overlay-example"> Delete </Tooltip>}>
                                                                            <button className='border p-1 px-2 ms-3 bt_color_hover bg-white' onClick={removePaymentType}>
                                        ...
                                        <div>
                                            <EditRecordForm parentEditRecord={parentEditRecord} childEmptyEditRecord={childEmptyEditRecord} editData={editData} close={closeditModel} />
                                        </div>
