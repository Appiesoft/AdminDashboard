...
import { bannerCreate } from '../../../../redux/appSettings/banner/actions';
import ToastHandle from '../../../../helpers/toastMessage';
import MainLoader from '../../../../components/MainLoader';

const AddBannerForm = ({ showBtn }) => {
    const dispatch = useDispatch()
    const store = useSelector(state => state)
    const bannerCreateStatus = store?.BannerCreate?.status
    const bannerCreateMessage = store?.BannerCreate?.message
    const bannerCreateLoader = store?.BannerCreate
    ...
    useEffect(() => {
        if (bannerCreateStatus) {
            ToastHandle('success', bannerCreateMessage);
            showBtn()

        } else if (bannerCreateStatus === false) {
            ToastHandle('error', bannerCreateMessage);
        }
    }, [bannerCreateStatus]);
        ...
        <>
            <Card>
                <Card.Body>
                    ...
                    {bannerCreateLoader?.loading ? <MainLoader />
                        :
                        <Form noValidate
                            ...
                            <Row className='p-3 mt-3 border'>
                                <Col lg={12}>
                                    ...
                                    <Row>
                                        <Col lg={12} className="mx-auto d-flex justify-content-center">
